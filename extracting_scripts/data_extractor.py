    ...
    if len(sheets) > 1:
        for sheet in sheets:
            ...
            if json_output:
                ...
                # pass
                # count = 0
                # for upazilla in json_output['upazillas']:
                #     count += 1
                #     print(f'Total Upazilla: {upazilla}')
        ...
        if json_output:
            ...
            # count = 0
            # for upazilla in json_output['upazillas']:
            #     if type(upazilla['name']) is str and not upazilla['name'].startswith('WARD'):
            #         count += 1
            #         # print(f'Union: {union}')
            #         # print(f"Name: {union['name']}, Code: {union['geo_code']}")
            #         # print(f'Upazilla {upazilla}')
            # print(f'Total Upazillas {count}')
            # for parts in json_output:
            #     print(parts)
            #     save_json(json_output[parts], parts)
...
for x in p.iterdir():
    if x.is_dir() and x.name == 'bbs_docs':
        for y in x.iterdir():
            if y.is_file() and y.name=='khulna.xlsx':
                prepare_data(y)


'''
    One sheet : barisal, ctg, mymen, raj
    Multiple sheets: dhaka, khulna, rangpur, sylhet
'''